...
- **Kolor**: zielony/czerwony dla wyniku okazji + kropek sygnałów (logika kontrariańska). Zmiana ceny 24h w nagłówku była początkowo **wyciszona szarym**, ale **od 2026-06-28 na życzenie użytkownika jest kolorowana giełdowo (wzrost=zielony, spadek=czerwony)** — akceptujemy, że w dniu spadku czerwone „−1,3% 24h" stoi obok zielonego wyniku okazji (klasy `.chg-pos`/`.chg-neg`, helper `chgClass` w index.html).
